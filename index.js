...
async function run() {
  try {
    ...
    app.put("/users/:email/photo", async (req, res) => {
      const email = req.params.email;
      const filter = { email: email };
      const options = { upsert: true };
      const updateUser = req.body;
    ...
    app.post("/allParcel", async (req, res) => {
      ...
      const isExist = await usersCollection.findOne({
        phone: newParcel.receiversNumber,
      });
      if (isExist) {
      ...
      return res.status(400).send({ message: "Receiver number not exists" });
    });
    ...
    app.patch("/calculateAvgRating/:id", async (req, res) => {
      ...
      const aggregationResult = await feedbackCollection
        .aggregate([
          {
    ...
    app.get("/topDeliveryMan", async (req, res) => {
      const topDeliveryMan = await usersCollection
        .aggregate([
          {
            $match: {
              role: "deliveryMan",
